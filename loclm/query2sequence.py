...
class ContextSequenceLearner:
    ...
    def __init__(self, graph=None, all_tokens=None):
        ...
        if graph is not None and all_tokens is not None:
            ...
            self.W = np.random.randn(
                self.vocab_size, self.vocab_size * 2) * 0.1
            self.b = np.zeros((self.vocab_size, 1))

    def _get_context_vector(self, query_dictionary):
        """Recursively flattens tree query_dictionary into a multi-hot vector."""
        vec = np.zeros((self.vocab_size, 1))

        def traverse(node):
            if isinstance(node, dict):
                for k, v in node.items():
                    if k in self.token_to_idx:
                        vec[self.token_to_idx[k]] = 1
                    traverse(v)
            elif isinstance(node, str):
                if node in self.token_to_idx:
                    vec[self.token_to_idx[node]] = 1
        traverse(query_dictionary)
        return vec
    ...
    def train(self, training_pairs, epochs=200, lr=0.05):
        for epoch in range(epochs):
            loss = 0
            for query_dictionary, target_sequence in training_pairs:
                context_vec = self._get_context_vector(query_dictionary)
                ...
                for i in range(len(target_sequence)):
                    if i == 0:
                        ...
                        prev_state = np.zeros((self.vocab_size, 1))
                        prev_state[self.token_to_idx[target_sequence[i-1]]] = 1

                    x = np.vstack((context_vec, prev_state))
                    target_idx = self.token_to_idx[target_sequence[i]]
    ...
    def predict(self, query_dictionary):
        """Op(G, query_dictionary) -> S. query_dictionary is the only input."""
        context_vec = self._get_context_vector(query_dictionary)
...
if __name__ == "__main__":
    ...
    fileslist = ['traction_force_calculation']
    
    G = {
        "image process heatmap": ["calculate traction"],
        "image process beads": ["calculate traction"]
    }
    ...
    # tokens = {"image", "calculate displacement", "query", "traction",
            ...
            # "image process cell", "calculate boundary"}

    # Example Training Data (Tree query_dictionary, Sequence S)
    
    train_set = []
    for filename in fileslist:
        with open(f"q2sdata/{filename}.json", "r") as f:
            data = json.load(f)
            for entry in data:
                train_set.append((
                    entry["input"],
        ...
        model.train(train_set, epochs=5000)
